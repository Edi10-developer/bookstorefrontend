...
export class MaintenanceComponent implements OnInit {
  ...
  constructor(private crud: CrudService,private modalService: NgbModal, private route :ActivatedRoute,) { 
    ...
    if(this.isbn !=''){
      this.crud.findAll('books/findbyid/'+this.isbn)
      .then((data:any)=>{
        console.log(data);
  ...
  save() {
    ...
    console.log(data);
    if (!this.form.valid) {
      this.message = 'Invalid entry, please fill in all fields';
      return;
    }
    if(this.isbn==null){
    this.crud.postAll('books/save', data)
      .then((e: any) => {
      ...
      //update
      this.crud.update('books/update', data)
      .then((e: any) => {
        this.message = e.message;
        alert(e.message);
        this.findAllBooks();
  ...
  findAllBooks() {
    this.crud.findAll("books/findall")
      .then((e: any) => {
        this.data = e;
  ...
  deleteBook(id) {
    if (confirm('DELETE BOOK?')) {
      this.crud.delete("books/delete/" + id)
        .then((e: any) => {
          alert(e.message);
          this.findAllBooks();
        })
